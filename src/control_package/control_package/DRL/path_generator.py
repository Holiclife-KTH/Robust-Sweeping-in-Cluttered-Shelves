...
class ReachPolicy(Node):
    ...
    def __init__(self, fail_quietly: bool = False, verbose: bool = False):
        ...
        self.robot = URSweepPolicy(
            model_path="/home/irol/workspace/project_th/src/ur5e_sweep/resource/251210"
        )
        ...
        self.cmd_log = []  # Log for RTDE commands
    ...
    def step_callback(self):
        ...
        if not np.array_equal(self.robot.current_tcp_pose, np.zeros(7)):
            ...
            if joint_pos is not None:
                ...
                # Log the command
                self.cmd_log.append([time.time()] + cmd)
    ...
    def save_log(self):
        if not self.cmd_log:
            self.get_logger().info("No commands to log.")
            return

        filename = f"rtde_cmd_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        # Save in the current directory or a specific log directory
        ...
        try:
            with open(filepath, "w", newline="") as csvfile:
                ...
                # Data
                writer.writerows(self.cmd_log)
            self.get_logger().info(f"RTDE command log saved to {filepath}")
        except Exception as e:
