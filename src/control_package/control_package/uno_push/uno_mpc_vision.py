...
class UnoMPC(UnoPush):
    ...
    def _vision_callback(self, msg: PoseStamped):
        if self._object_manager.np_target_object_pose is None:
            return

        current_tcp_pose = self._controller.current_tcp_pose
        target_object_pose = self._object_manager.np_target_object_pose

        # 현재 상태와 참조 경로 계산
        current_state = target_object_pose[:2]
        ref_path = self._calculate_sine_target_path(self._direction)

        # MPC로 행동 결정
        action = self._mpc.get_action(current_state, ref_path)

        # 행동 실행
        motion_real = self._push(action[0], action[1], msg)

        # 모델 업데이트
        self.__f_model.update(action, motion_real)
        self.__i_model.update(motion_real, action)
